				...
				delegate: Column {
					...
					Item {
						...
						Image {
							...
							source: "/Mentalgami/Asset/Image/CourseCover.png"
...
//						fontSizeMode: Text.HorizontalFit
		...
		delegate: ItemDelegate {
			...
			function stack()
			{
				var render = SchemeFactory.load("Mentalgami.Meta.TrackRender", trackLayout.scheme);
				if (!render) {
					return false;
				}
				let component_url = undefined;
				component_url = render.makeURL(Mentalgami.StackviewLayout);
				...
				component_url = render.makeURL(Mentalgami.PopupviewLayout);
				if (component_url.toString().length > 0) {
			...
			contentItem: Item {
				...
				TrackLayout {
					...
					Component.onCompleted: {
						represent(content_type, Mentalgami.PlaylistLayout);
					}
