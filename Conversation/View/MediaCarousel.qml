...
ListView {
	...
	delegate: MessageLayout {
		...
		Component.onCompleted: {
			render(scheme, Conversation.VerticalLayout);
		}
		...
		onRendered: {
			load(application, id, uid, reply_id, author_subject_id, body, new Date(post_timestamp), new Date(modification_timestamp));
		}
