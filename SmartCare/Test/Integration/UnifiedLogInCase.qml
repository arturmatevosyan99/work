...
import "qrc:/SmartCare/View/Application" as SmartCareView
...
PetrI {
	...
	property Component tokenComponent: Component {
		Token {
			...
			property SmartCareView.AuthorizationQuiz authorizationQuiz: SmartCareView.AuthorizationQuiz {
				avatar: token.avatar
			}
		}
	}
	...
	Assertion {
		...
		Assert {
			...
			onActionPrepared: {
				...
				if (currentToken.authorizationQuiz.phase !== SmartCareView.AuthorizationQuiz.Phase.SignIn) {
	...
	Assertion {
		...
		Assert {
			...
			onActionPrepared: {
				...
				if (currentToken.authorizationQuiz.phase !== SmartCareView.AuthorizationQuiz.Phase.CodeUp) {
					console.error("Wrong phase at this stage of authorization:", currentToken.authorizationQuiz.phase);
				}
			}
		}
