...
namespace Cadabra { namespace Queen { namespace SmartCare { namespace View {
	...
	void TrackLayout::render(Cadabra::Queen::Core::Meta::Qd scheme, TrackRenderLayout type)
	{
		...
		//Cadabra::Queen::SmartCare::TrackLayout type
		Core::Meta::Scheme<SmartCare::Meta::TrackRender>::Iterator r(Core::Meta::Scheme<SmartCare::Meta::TrackRender>::Iterate(scheme));
		QQmlComponent component(
								qmlContext(this)->engine(),
								Core::Meta::Scheme<Meta::TrackRender>::Exists(r) ? r->makeURL(type) : QUrl("qrc:/SmartCare/View/Track/UnsupportedTrack.qml"),
	...
	Cadabra::Queen::SmartCare::Model::AbstractTrack* TrackLayout::getModel() const
	{		
		return _view ? _view->getModel() : nullptr;
	}

	Cadabra::Queen::SmartCare::View::TrackView* TrackLayout::getView() const
	{
		return _view;
	}
