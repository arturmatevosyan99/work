...
namespace Cadabra { namespace Queen { namespace SmartCare { namespace Net {
	...
	MojiFile::State MojiFile::load(const QString& collection)
	{
		...
		_collection = collection;
//		load("smartcare", _family, collection, "cmz"); /// < smartcare/awardji/com.oblozhko.metabolicplate.cmz
		const State retval(Core::Net::QRCFile::load("SmartCare", QString("%1.%2.cmz").arg(collection, _family)));
