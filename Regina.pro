...
ios {
	OTHER_FILES += $$PWD/regina.ios-asset/Info.plist
	QMAKE_INFO_PLIST = $$PWD/regina.ios-asset/Info.plist
	DISTFILES += $$PWD/regina.ios-asset/Info.plist

	ENTITLEMENTS.name = CODE_SIGN_ENTITLEMENTS
	CONFIG(release, debug|release) {
		ENTITLEMENTS.value = $$PWD/regina.ios-asset/Production.entitlements
		DISTFILES += $$PWD/regina.ios-asset/Production.entitlements
	} else {
		ENTITLEMENTS.value = $$PWD/regina.ios-asset/Development.entitlements
		DISTFILES += $$PWD/regina.ios-asset/Development.entitlements
	}
	QMAKE_MAC_XCODE_SETTINGS += ENTITLEMENTS

	IOS_LAUNCH_IMAGES.files += $$PWD/regina.ios-asset/ReginaLauncher.png
	IOS_LAUNCH_IMAGES.files += $$PWD/regina.ios-asset/ReginaLauncher.xib
	QMAKE_BUNDLE_DATA += IOS_LAUNCH_IMAGES

	QMAKE_ASSET_CATALOGS_APP_ICON = "Icon"
	QMAKE_ASSET_CATALOGS = $$PWD/regina.ios-asset/Images.xcassets
} android {
	ANDROID_PACKAGE_SOURCE_DIR = $$PWD/regina.android
}
